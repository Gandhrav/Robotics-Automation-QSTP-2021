...
class Unicycle:
    ...
    def step(self, v: float, w: float, n: int):
        ...
        for i in range(1,n+1):
            theta+=w*self.dt*i
            x+=v*i*self.dt*np.math.cos(self.theta*(np.pi/180))
            y+=v*i*self.dt*np.math.sin(self.theta*(np.pi/180))
...
if __name__ == "__main__":
    ...
    uni_1 = Unicycle(0,0,0,0.1)
    uni_1.step(1,0.5,25)
    uni_1.plot(1,0.5)

    uni_2 = Unicycle(0,0,1.57,0.2)
    uni_2.step(0.5,1,10)
    uni_2.plot(0.5,1)
    
    uni_3 = Unicycle(0,0,0.77,0.05)
    uni_3.step(5,4,50)
    uni_3.plot(5,4)
